...
interface ChatAttachment { name: string; path: string; size: number; type: string }
...
interface Participant { id: number; name: string; role?: string | null; avatar_url?: string | null }
interface ChatState {
  thread_id: number;
  channel: string;
...
function Avatar({ name, url, id, size = 32 }: { name: string; url?: string | null; id: number; size?: number }) {
...
function renderContent(text: string, names: string[]): React.ReactNode {
  ...
  // Longest names first so "@Shekhar Mazumdar" wins over a bare "@Shekhar".
...
export function ProjectChat({ projectId }: { projectId: number | string }) {
  ...
  useEffect(() => {
    ...
    api.getProjectChat(projectId)
      .then((data: ChatState) => {
        if (!active) return;
        setState(data);
        setTimeout(() => scrollToBottom(false), 50);
        const lastId = data.messages.length ? data.messages[data.messages.length - 1].id : 0;
        if (lastId) api.markProjectChatRead(projectId, lastId).catch(() => {});
      })
      .catch((e: unknown) => {
        if (!active) return;
  ...
  useEffect(() => {
    ...
    channel.listen(".chat", (e: { action: string; data: any }) => {
      if (e.action === "message.sent") {
        ...
        // Clear the sender's typing indicator once their message lands.
        setTypingUsers((t) => { const n = { ...t }; delete n[e.data.author_id]; return n; });
        if (near) setTimeout(() => scrollToBottom(), 60);
        api.markProjectChatRead(projectId, e.data.id).catch(() => {});
    ...
    return () => { try { echo.leave(state.channel); } catch { /* noop */ } channelRef.current = null; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state?.channel, projectId, meId]);
  ...
  const send = async () => {
    ...
    try {
      const msg: ChatMessage = await api.sendProjectChat(projectId, { content, mentions, attachments: attached });
      setState((s) => (s && !s.messages.some((m) => m.id === msg.id) ? { ...s, messages: [...s.messages, msg] } : s));
      setTimeout(() => scrollToBottom(), 60);
    } catch {
      setDraft(content); setFiles(attached); // restore on failure
  ...
  const saveEdit = async (id: number) => {
    ...
    if (!text) return;
    try {
      const updated: ChatMessage = await api.editProjectChatMessage(projectId, id, text);
  ...
  const remove = async (id: number) => {
    ...
    try { await api.deleteProjectChatMessage(projectId, id); } catch { /* noop */ }
  };

  /* ── Read receipts: who has read up to the last message ── */
  ...
  if (forbidden) return (
    <div className="flex h-64 flex-col items-center justify-center text-center">
      ...
      <p className="mt-3 text-sm font-semibold">Restricted discussion</p>
      <p className="mt-1 max-w-md text-sm text-muted-foreground">Only people assigned to this matter can view its chat.</p>
    </div>
  );
  if (!state) return <p className="py-10 text-center text-sm text-muted-foreground">This case chat is unavailable.</p>;

  return (
    <div className="flex h-[calc(100vh-260px)] min-h-[420px] flex-col rounded-md border border-border bg-background">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
        ...
        <span className="text-[11px] font-medium text-muted-foreground">Case Discussion</span>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 space-y-1 overflow-y-auto px-4 py-3">
        {messages.length === 0 && (
          <p className="py-10 text-center text-sm text-muted-foreground">No messages yet. Start the discussion for this matter.</p>
        )}
        {messages.map((m, i) => {
                    ...
                    <div className="relative">
                      ...
                      {m.attachments?.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-2">
                          {m.attachments.map((a) => (
                            <button key={a.path} onClick={() => api.downloadChatAttachment(projectId, a.path, a.name)}
      ...
      <div className="relative border-t border-border p-3">
        ...
        <div className="flex items-end gap-2">
          ...
          <textarea value={draft} onChange={(e) => onDraftChange(e.target.value)}
            ...
            rows={1} placeholder="Message the case team…  (@ to mention)"
